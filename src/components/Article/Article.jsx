const Article = ({ title, text, tags, image, alt }) => {
    return (
        <div className="p-5 bg-gray-200 dark:bg-dark-200 sm:rounded-xl sm:shadow-lg flex flex-col gap-2 items-center ">
            ...
            <div className="w-full flex-row justify-end gap-2 pr-5 hidden sm:block">
                {
                    tags.map(tag => <span key={tag} className="bg-alura-100 dark:bg-dark-100 px-4 py-2 rounded-full text-gray-200  text-xs font-bold uppercase">{tag}</span>)
                }
            </div>
